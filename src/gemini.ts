...
/**
 * Sends a message to the Gemini 1.5 Flash model with history context.
 * Uses the agency knowledge base as a system instruction.
 */
export async function askGemini(messageText: string, history: GeminiMessage[] = []): Promise<string> {
  ...
  try {
    const modifiedInstruction = config.systemInstruction + 
